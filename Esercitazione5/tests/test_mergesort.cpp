...
// adatto i test fatti nell'esercitazione precedente a MergeSort:
...
int main(void) {
	...
	for (int i=1; i<101; i++) {
	    ...
	    merge_sort(v1, 0, v1.size()-1);  // applico l'algoritmo di merge_sort che ho definito nell'header
	
	    if (!is_sorted(v1)) {
	    	std::cout << "ERRORE: l'algoritmo MergeSort non ha funzionato correttamente sul vettore di double." << "\n";
		    return EXIT_FAILURE; 
	    }
	}
	...
	for (int i=1; i<101; i++) {
		...
		if (!is_sorted(v2)) {
	    	std::cout << "ERRORE: l'algoritmo MergeSort non ha funzionato correttamente sul vettore di interi." << "\n";
		    return EXIT_FAILURE; 
	    }
	}
	...
	if (!is_sorted(s)) {
	    	std::cout << "ERRORE: l'algoritmo MergeSort non ha funzionato correttamente sul vettore di stringhe." << "\n";
		    return EXIT_FAILURE; 
	    }
